...
function OrderItem() {
  ...
  const url = 'http://localhost:8000/api/orderitems'
  ...
  const fetchOrderitems = async () => {
      await axios.get(`http://localhost:8000/api/orderitems`).then(({data})=>{
          setOrderitems(data)
      })
  }
  ...
  const deleteOrderitems = async (id) => {
      const isConfirm = await Swal.fire({
        ...
        await axios.delete(`http://localhost:8000/api/orderitems/${id}`).then(({data})=>{
          Swal.fire({
              icon:"success",
              text:data.message
          })
            ...
            <div className="layout-page">
              ...
              <div className="content-wrapper">
                ...
                <div className="container-xxl flex-grow-1 container-p-y">
                  ...
                  <div className="card">
                    <h5 className="card-header">Order Item List</h5>
                                ...
                                orderitems.length > 0 && (
                                    orderitems.map((row, key)=>(
                                        <tr key={key}>
                                            ...
                                            <td>
                                            <Button to={`/orderitems/show`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEye size={16} color="white"  />
                                            </Button>
                                            <Button to={`/orderitems/edit/${row.id}`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEdit size={16} color="white"  />
                                            </Button>
                                            <Button style={{ backgroundColor:'indigo' }} onClick={()=>deleteOrderitems(row.id)}>
