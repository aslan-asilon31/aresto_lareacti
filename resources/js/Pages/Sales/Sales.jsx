...
function Sales() {
  ...
  const fetchSales = async () => {
      await axios.get(`http://localhost:8000/api/sales`).then(({data})=>{
          setSales(data)
      })
  }
  ...
  const deleteSales = async (id) => {
      const isConfirm = await Swal.fire({
        ...
        await axios.delete(`http://localhost:8000/api/sales/${id}`).then(({data})=>{
          Swal.fire({
              icon:"success",
              text:data.message
          })
            ...
            <div className="layout-page">
              ...
              <div className="content-wrapper">
                ...
                <div className="container-xxl flex-grow-1 container-p-y">
                  ...
                  <div className="card">
                    <h5 className="card-header">Sales List</h5>
                    <div className=" text-nowrap">
                      <div className="m-3">
                        <div className="col-12 text-white d-flex">
                          ...
                          <Button className='btn mb-3 text-white float-end  ml-3 me-2' to={"/sales/create"} style={{ backgroundColor: 'indigo' }}>
                                    ...
                                    sales.length > 0 && (
                                        sales.map((row, key)=>(
                                            <tr key={key}>
                                                ...
                                                <td>{row.status}</td>
                                                <td>
                                                <Button to={`/sales/show`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                    <FaEye size={16} color="white"  />
                                                </Button>
                                                <Button to={`/sales/edit/${row.id}`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                    <FaEdit size={16} color="white"  />
                                                </Button>
                                                <Button style={{ backgroundColor:'indigo' }} onClick={()=>deleteSales(row.id)}>
