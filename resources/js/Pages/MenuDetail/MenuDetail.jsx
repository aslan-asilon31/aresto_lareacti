...
function MenuDetail() {
  ...
  const url = 'http://localhost:8000/api/menudetails'

  useEffect(() => {
    const fetchMenuDetails = async function () {
      try {
        setLoading(true);
  ...
  const fetchMenuDetails = async () => {
      await axios.get(`http://localhost:8000/api/menudetails`).then(({data})=>{
          setMenuDetails(data)
      })
  }


  const deleteMenuDetails = async (id) => {
      const isConfirm = await Swal.fire({
        ...
        await axios.delete(`http://localhost:8000/api/menudetails/${id}`).then(({data})=>{
          Swal.fire({
              icon:"success",
              text:data.message
          })
            ...
            <div className="layout-page">
              ...
              <div className="content-wrapper">
                ...
                <div className="container-xxl flex-grow-1 container-p-y">
                  ...
                  <div className="card">
                    <h5 className="card-header">Menu Detail List</h5>
                    <div className="table-responsive text-nowrap">
                      <div className="m-3">
                        <div className="col-12 text-white d-flex">
                          ...
                          <a className='btn mb-3 text-white float-end  ml-3 me-2' href={"/menudetails/create"} style={{ backgroundColor: 'indigo' }}>
                          ...
                          <tbody>
                            {
                                menudetails.length > 0 && (
                                    menudetails.map((row, key)=>(
                                        <tr key={key}>
                                            <td>{row.menu_id}</td>
                                            <td>{row.name}</td>
                                            <td>
                                                <img width="50px" src={`http://localhost:8000/storage/menu_details/image/${row.image}`} />
                                            </td>
                                            <td>{row.desc}</td>
                                            <td>{row.slug}</td>
                                            <td>{getFormattedDate(row.created_at)} , {getFormattedYear(row.created_at)}</td>
                                            <td>
                                            <Button to={`/menudetails/show`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEye size={16} color="white"  />
                                            </Button>
                                            <Button to={`/menudetails/edit/${row.id}`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEdit size={16} color="white"  />
                                            </Button>
                                            <Button style={{ backgroundColor:'indigo' }} onClick={()=>deleteMenuDetails(row.id)}>
