...
function Shift() {
  ...
  const url = 'http://localhost:8000/api/shifts'
  ...
  const fetchShifts = async () => {
      await axios.get(`http://localhost:8000/api/shifts`).then(({data})=>{
          setShifts(data)
      })
  }
  ...
  const deleteShifts = async (id) => {
      const isConfirm = await Swal.fire({
        ...
        await axios.delete(`http://localhost:8000/api/shifts/${id}`).then(({data})=>{
          Swal.fire({
              icon:"success",
              text:data.message
          })
            ...
            <div className="layout-page">
              ...
              <div className="content-wrapper">
                ...
                <div className="container-xxl flex-grow-1 container-p-y">
                  ...
                  <div className="card">
                    <h5 className="card-header">Shift List</h5>
                    <div className="table-responsive text-nowrap">
                      <div className="m-3">
                        <div className="col-12 text-white d-flex">
                          ...
                          <Button className='btn mb-3 text-white float-end  ml-3 me-2' to={"/shifts/create"} style={{ backgroundColor: 'indigo' }}>
                                ...
                                shifts.length > 0 && (
                                    shifts.map((row, key)=>(
                                        <tr key={key}>
                                            ...
                                            <td>{getFormattedDate(row.start_time)} , {getFormattedYear(row.start_time)}</td>
                                            <td>{getFormattedDate(row.end_time)} , {getFormattedYear(row.end_time)}</td>
                                            <td>{getFormattedDate(row.created_at)} , {getFormattedYear(row.created_at)}</td>
                                            <td>
                                            <Button to={`/shifts/show`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEye size={16} color="white"  />
                                            </Button>
                                            <Button to={`/shifts/edit/${row.id}`} className='btn btn-success me-2' style={{ backgroundColor:'indigo' }}>
                                                  <FaEdit size={16} color="white"  />
                                            </Button>
                                            <Button style={{ backgroundColor:'indigo' }} onClick={()=>deleteShifts(row.id)}>
